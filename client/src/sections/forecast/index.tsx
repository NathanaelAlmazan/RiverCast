...
export default function Forecast() {
  ...
  const [{ apiData: current, loading: currentLoader }] = useGetDataApi<
    CurrentWeather[]
  >('https://rivercast.automos.net/api/current', []);
  const [{ apiData: forecast, loading: forecastLoader }] = useGetDataApi<
    ForecastWeather[]
  >('https://rivercast.automos.net/api/forecast', []);
  const [{ apiData: history, loading: historyLoader }, { setData }] =
    useGetDataApi<HistoricalWeather[]>(
      'https://rivercast.automos.net/api/history?year=2023',
      [],
    );
  ...
  useEffect(() => {
    ...
    if (stationData) {
      const bias = station === "NANGKA" ? 0 : 0.3;
      setWeather([
        {
          id: 1,
          amount: `${(stationData.TEMPERATURE + bias).toFixed(2)}°C`,
          type: 'Temperature',
          icon: '/assets/images/dashboard/ic_temperature.svg',
        },
        {
          id: 2,
          amount: `${(stationData.HUMIDITY - bias).toFixed(2)}g/kg`,
          type: 'Humidity',
          icon: '/assets/images/dashboard/ic_humidity.svg',
        },
        {
          id: 3,
          amount: `${(stationData.RAINFALL + bias).toFixed(2)}mm/hr`,
          type: 'Precipitation',
          icon: '/assets/images/dashboard/ic_rainfall.svg',
        },
        {
          id: 4,
          amount: `${(stationData.WIND_SPEED - bias).toFixed(2)}m/s ${stationData.WIND_DIRECTION}`,
      ...
      setCurrentLevel(
        stationData.WATERLEVEL > 0 ? stationData.WATERLEVEL : 12.21,
      );
    }
  }, [current, station]);
  ...
  useEffect(() => {
    ...
    setRiverlevel(
      stationData.map((data, idx) => ({
        name: new Date(data.TIMESTAMP).toLocaleDateString(undefined, {
          weekday: 'short',
        }),
  ...
  useEffect(() => {
    const groupedForecast = history.reduce<{ [key: string]: number }>(
      (acc, obj) => {
        const month = new Date(obj.TIMESTAMP).toLocaleDateString(undefined, {
          month: 'short',
        });
    ...
    const groupedActual = history.reduce<{ [key: string]: number }>(
      (acc, obj) => {
        const month = new Date(obj.TIMESTAMP).toLocaleDateString(undefined, {
          month: 'short',
        });
    ...
    setHistorical(
      Object.keys(groupedActual).map((key) => ({
        Datetime: key,
  ...
  const handleChangeYear = (year: number) => {
    getDataApi<HistoricalWeather[]>(
      `https://rivercast.automos.net/api/history?year=${year}`,
        ...
        <AppAnimate animation='transition.slideUpIn' delay={200}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={4}>
              <Stack spacing={5}>
                <TemperatureCard station={station} forecasts={temperatures} />
